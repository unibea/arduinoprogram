...
int colorcheck() {
  ...
  Serial.print("Red   = ");
  Serial.println(get_vec[0]);
  Serial.print("Green = ");
  Serial.println(get_vec[1]);
  Serial.print("Blue  = ");
  Serial.println(get_vec[2]);
  Serial.print("R_to_getColor");
  Serial.println(R_to_getColor);
  Serial.print("B_to_getColor");
  Serial.println(B_to_getColor);
  Serial.print("Y_to_getColor");
  Serial.println(Y_to_getColor);
...
void tracet(float Kp) {
  ...
  HzSPL = spchL(380 + outL);
  HzSPR = spchR(380 + outR);
...
int tyokusin(int spl, int Step) {
  ...
  Timer3.initialize(spchL(spl - 10));  //左//-10は機体の偏差分を考慮
  Timer1.attachInterrupt(timerPulse1);
  Timer3.attachInterrupt(timerPulse2);
  while (1) {
    Serial.print('\t');
...
int tyokusinS() {
  ...
  Timer3.initialize(spchL(600 - 10));  //-10は機体の偏差分を考慮
  Timer1.attachInterrupt(timerPulse1);
  Timer3.attachInterrupt(timerPulse2);
  while (1) {
    S0 = analogRead(LEFT);    //0番ﾋﾟﾝ
...
void kaisyuu() {
  xai = 0;
  int ST = 0, Flag = 0, ex = 0;  //ST:ボールが入った後のループ時間計測　Flag:ボール認識後STをオンにするため ex:もしボールが取れてなければ無限ループ回避のため
  Timer1.initialize(spchR(150));
  Timer3.initialize(spchL(150));
  Timer1.detachInterrupt();
  Timer3.detachInterrupt();
  Serial.print("11");
  StepL = 0;
