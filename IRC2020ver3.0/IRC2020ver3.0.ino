...
void loop() {
  ...
  switch (phase) {
    ...
    case 2:
      ...
      if (count == 5) {
        while (1) {
          tracet(1.0);
          if (GET <= 4 && StepL > 300) {
            Serial.print("GET1,GET=");
            Serial.println(GET);
            break;
          } else if (GET >= 5 && GET <= 8 && StepL > 700) {
            Serial.println("GET2,GET=");
            Serial.println(GET);
            break;
          } else if (GET >= 9 && GET <= 12 && StepL > 1200) {
            Serial.print("GET3,GET=");
            Serial.println(GET);
            break;
          } else if (GET >= 13 && StepL > 1700) {
            Serial.print("GET4,GET=");
            Serial.println(GET);
            break;
          }
        }
        kousin(400, 300);  //後ろの壁当てをすると長距離走ることになりずれるため出来るだけライン上からはなれないようトレースして少し進み戻る。
        phase = 3;
      }
      break;
    case 3:
      Serial.println("ball serch");
      phase = tansaku();
      break;
    case 4:
      Serial.println("ball In");
      kaisyuu();
      Frag = 0;
      ball++;
      phase = 5;
    ...
    case 7:
      ...
      if (iro == kakoiro) {
        phase = 6;  //同じ色の場合
        Serial.println("同じ色のボール");
      }

      if (iro == 7 && (kakoiro == 6 || kakoiro == 8)) {
        sortb();  //黄色、赤色⇨青色
        Serial.println("黄色、赤色⇨青色");
      }

      if (iro == 6 && kakoiro == 7) {
        sortg(1.0);  //青色⇨赤色
        Serial.println("青色→赤色");
      }
      if (iro == 8 && kakoiro == 7) {
        sortg(2.0);  //青色⇨黄色
        Serial.println("青色→黄色");
      }

      if (iro == 8 && kakoiro == 6) {
        sortRY();  //赤色⇨黄色
        Serial.println("赤色→黄色");
      }
      if (iro == 6 && kakoiro == 8) {
        sortYR();  //黄色⇨赤色
        Serial.println("黄色→赤色");
      }
